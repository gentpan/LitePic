...
function initLicenseDialog() {
    ...
    trigger.addEventListener('click', () => {
        ...
        const version = String(window.LITEPIC_VERSION || '3.6.0');
...
/* ===== files.js ===== */
/**
 * 文件浏览(/files)—— WebDAV 目录树的 Finder 式列视图。
 *
 * 状态就是一个「列栈」:columns[0] 永远是根,columns[i+1] 是在 columns[i]
 * 里被点开的那个文件夹。点文件夹 = 截断到该列之后再压一列;点面包屑 =
 * 截断到对应深度。所有变更(移动/复制/删除/改名)完成后整栈重载 ——
 * 列最多五六个、每列一次带索引的查询,比逐列做增量补丁可靠得多,
 * 也不会跟 dav_entries 的惰性 reconcile 打架。
 *
 * 选择限定在单列内,跟 Finder 一致:跨列多选在列视图里没有可表达的语义。
 */
(() => {
    'use strict';

    const columnsEl = document.querySelector('[data-fb-columns]');
    if (!columnsEl) return;

    const config = window.LITEPIC_FB || {};
    const DIR_UNFILED = config.dirUnfiled || '未分类';

    const els = {
        columns: columnsEl,
        crumbs: document.querySelector('[data-fb-crumbs]'),
        preview: document.querySelector('[data-fb-preview]'),
        selection: document.querySelector('[data-fb-selection]'),
        selectionCount: document.querySelector('[data-fb-selection-count]'),
        deleteBtn: document.querySelector('[data-fb-delete]'),
        clearBtn: document.querySelector('[data-fb-clear]'),
        newFolderBtn: document.querySelector('[data-fb-new-folder]'),
        refreshBtn: document.querySelector('[data-fb-refresh]'),
    };

    /** @type {Array<{path:string,node:object,entries:object[],total:number,hasMore:boolean,el:HTMLElement,listEl:HTMLElement}>} */
    let columns = [];
    /** 选中项的 path,只在 selectionDepth 这一列内。 */
    let selection = new Set();
    let selectionDepth = -1;
    let lastClickedPath = '';
    let busy = false;

    const notify = (message, type) => window.ImgEt?.Utils?.showNotification?.(message, type);
    ...
    /** 一个可写的投放目标:相册文件夹和「未分类」,不含只读的日期视图。 */
    const isDropTarget = (entry) => entry.is_collection
        && entry.writable
        && (entry.kind === 'album' || entry.kind === 'unfiled');

    // ---- API ----------------------------------------------------------------

    async function apiList(path, offset = 0) {
        ...
        if (!res.ok || data.status !== 'success') {
            throw new Error(data.message || `读取目录失败 (${res.status})`);
        }
        return data;
    }

    async function apiPost(formAction, payload) {
        const res = await fetch('/api/v1/files', {
            ...
            body: JSON.stringify({ form_action: formAction, csrf_token: csrfToken(), ...payload }),
        });
        const data = await res.json().catch(() => ({}));
        if (!res.ok || data.status !== 'success') {
    ...
    /** 把逐项失败汇总成一条能看懂的提示。 */
    function reportOutcome(done, failed, verb) {
        if (done > 0) {
            notify(`已${verb} ${done} 项`, failed.length ? 'warning' : 'success');
        }
        if (failed.length) {
            const head = failed.slice(0, 3).map((f) => f.message).join('；');
            notify(failed.length > 3 ? `${head}…（共 ${failed.length} 项失败）` : head, 'error');
        }
    }
    ...
    function iconFor(entry) {
        if (entry.kind === 'unfiled') return 'fa-folder-open';
        // fa-calendar 在 40px 格子里轮廓更清楚；fa-calendar-days 的日格缩小时易糊成横条。
        if (entry.kind === 'date') return 'fa-calendar';
        if (entry.kind === 'date_year' || entry.kind === 'date_month') return 'fa-folder';
        if (entry.kind === 'album') return entry.visibility === 'public' ? 'fa-folder-open' : 'fa-folder';
        return 'fa-image';
    }

    /** 正方形底托包 FA 图标，避免 <i> 自身被字体行高拉成竖条。 */
    function makeIconEl(iconName) {
        const box = document.createElement('span');
        box.className = 'fb-item-icon';
        box.setAttribute('aria-hidden', 'true');
        const glyph = document.createElement('i');
        glyph.className = `fa-light ${iconName}`;
    ...
    function buildRow(entry, depth) {
        ...
        if (entry.is_collection) {
            ...
            if (entry.missing) li.classList.add('is-missing');
        }
        if (!entry.writable) li.classList.add('is-readonly');

        // 图片行 / 有封面的相册：用缩略图。磁盘上已丢的文件别再请求一张必 404 的图。
        const thumbUrl = (!entry.missing && entry.thumb_url) ? entry.thumb_url : '';
        if (thumbUrl) {
            const img = document.createElement('img');
            img.className = 'fb-item-thumb';
            img.src = thumbUrl;
            img.alt = '';
            img.width = 40;
            img.height = 40;
            ...
            img.addEventListener('error', () => {
                img.replaceWith(makeIconEl(iconFor(entry)));
            });
            li.appendChild(img);
        } else {
            li.appendChild(makeIconEl(iconFor(entry)));
        }

        const name = document.createElement('span');
        name.className = 'fb-item-name';
        name.textContent = entry.name;
        name.title = entry.missing ? `${entry.name}（文件在磁盘上已丢失）` : entry.name;
        li.appendChild(name);
        ...
        if (entry.is_collection) {
            if (!entry.writable) {
                ...
                meta.textContent = String(entry.image_count ?? '');
            } else {
                meta.textContent = '';
            }
        } else {
            meta.textContent = entry.placeholder ? '空' : formatBytes(entry.size);
        }
        li.appendChild(meta);

        if (entry.is_collection) {
            const chevron = document.createElement('i');
            chevron.className = 'fb-item-chevron fa-light fa-chevron-right';
            chevron.setAttribute('aria-hidden', 'true');
            li.appendChild(chevron);
        }

        // 只读项(日期视图)不可拖:它没有 dav_entries 行,移不动也删不掉。
        li.draggable = entry.writable && entry.kind !== 'unfiled' && entry.kind !== 'date';

        return li;
    }

    function buildColumn(data, depth) {
        const col = document.createElement('div');
        col.className = 'fb-col';
        col.dataset.path = data.path;
        col.dataset.depth = String(depth);

        const head = document.createElement('div');
        head.className = 'fb-col-head';
        const headName = document.createElement('span');
        headName.textContent = data.node.name || '全部';
        const headCount = document.createElement('span');
        headCount.textContent = data.total ? `${data.total} 项` : '';
        ...
        appendEntries(column, data, depth);
        return column;
    }

    function appendEntries(column, data, depth) {
        column.entries.push(...data.entries);
        column.hasMore = !!data.has_more;

        column.listEl.querySelector('.fb-col-empty')?.remove();
        column.listEl.querySelector('.fb-col-more')?.closest('li')?.remove();

        for (const entry of data.entries) {
            column.listEl.appendChild(buildRow(entry, depth));
        }

        if (!column.entries.length) {
            const empty = document.createElement('li');
            empty.className = 'fb-col-empty';
            empty.textContent = column.node.kind === 'album' ? '相册里还没有图片' : '空目录';
            column.listEl.appendChild(empty);
            return;
        }
        ...
        if (column.hasMore) {
            ...
            const btn = document.createElement('button');
            btn.type = 'button';
            btn.className = 'fb-col-more';
            btn.textContent = `继续加载（已显示 ${column.entries.length} / ${column.total}）`;
            btn.addEventListener('click', () => loadMore(column, depth));
            more.appendChild(btn);
            column.listEl.appendChild(more);
        }
    }

    async function loadMore(column, depth) {
        const btn = column.listEl.querySelector('.fb-col-more');
        if (btn) {
            btn.disabled = true;
            btn.textContent = '加载中…';
        }
        try {
            const data = await apiList(column.path, column.entries.length);
            appendEntries(column, data, depth);
        } catch (error) {
    ...
    function renderCrumbs() {
        ...
        const crumbs = [{ name: '全部', path: '/' }];
        if (deepest && deepest.path !== '/') {
            let current = '';
            for (const segment of deepest.path.split('/').filter(Boolean)) {
                current += `/${segment}`;
        ...
        crumbs.forEach((crumb, index) => {
            if (index > 0) {
                ...
                sep.className = 'fb-crumb-sep fa-light fa-chevron-right';
            ...
            if (index === crumbs.length - 1) btn.classList.add('is-current');
    ...
    /** 高亮每一列里被点开的那个文件夹,以及当前列。 */
    function paintOpenTrail() {
        columns.forEach((column, index) => {
            const nextPath = columns[index + 1]?.path;
            column.el.classList.toggle('is-active', index === columns.length - 1);
            column.listEl.querySelectorAll('.fb-item').forEach((li) => {
                li.classList.toggle('is-open', !!nextPath && li.dataset.path === nextPath);
            });
        });
    }
    ...
    function paintSelection() {
        columns.forEach((column, index) => {
            column.listEl.querySelectorAll('.fb-item').forEach((li) => {
                li.classList.toggle(
                    'is-selected',
                    index === selectionDepth && selection.has(li.dataset.path)
                );
            });
        });

        const count = selection.size;
        if (els.selection) els.selection.hidden = count === 0;
        if (els.selectionCount) {
            els.selectionCount.textContent = count ? `已选中 ${count} 项` : '';
        }
        renderPreview();
    }

    function selectedEntries() {
        const column = columns[selectionDepth];
        if (!column) return [];
        return column.entries.filter((entry) => selection.has(entry.path));
    }
    ...
    function renderPreview() {
        ...
        if (picked.length !== 1 || picked[0].is_collection) {
        ...
        figure.className = 'fb-preview-figure';
        if (entry.thumb_url && !entry.missing && !entry.placeholder) {
            const img = document.createElement('img');
            ...
            img.loading = 'lazy';
            img.decoding = 'async';
            img.addEventListener('click', () => openLightbox(entry));
            figure.appendChild(img);
        } else {
            const blank = document.createElement('div');
            blank.className = 'fb-preview-blank';
            const icon = document.createElement('i');
            icon.className = 'fa-light fa-image-slash';
            const text = document.createElement('span');
            text.textContent = entry.placeholder
                ? '占位文件，还没有内容'
                : (entry.missing ? '文件在磁盘上已丢失' : '暂无缩略图');
            blank.append(icon, text);
            figure.appendChild(blank);
        }
        els.preview.appendChild(figure);

        const title = document.createElement('h3');
        title.className = 'fb-preview-name';
        title.textContent = entry.name;
        els.preview.appendChild(title);

        const meta = document.createElement('dl');
        meta.className = 'fb-preview-meta';
        const rows = [
            ['大小', entry.placeholder ? '—' : formatBytes(entry.size)],
            ['尺寸', entry.width && entry.height ? `${entry.width} × ${entry.height}` : '—'],
            ['格式', entry.mime || '—'],
            ['上传', formatDate(entry.created_at)],
            ['原名', entry.original_name || '—'],
            ['标识', entry.filename || '—'],
        ];
        ...
        if (entry.url) {
            ...
            copy.innerHTML = '<i class="fa-light fa-link" aria-hidden="true"></i><span>复制链接</span>';
            copy.addEventListener('click', () => copyToClipboard(entry.url));
        ...
        if (entry.writable) {
            ...
            rename.type = 'button';
            rename.className = 'btn btn--secondary btn--sm';
            rename.innerHTML = '<i class="fa-light fa-pen" aria-hidden="true"></i><span>重命名</span>';
            rename.addEventListener('click', () => startRename(entry));
            actions.appendChild(rename);
        }

        els.preview.appendChild(actions);
    }

    function openLightbox(entry) {
        const column = columns[selectionDepth];
        const urls = (column?.entries || [])
            .filter((item) => !item.is_collection && item.url && !item.missing)
            .map((item) => item.url);
        if (window.ViewImage && typeof window.ViewImage.display === 'function') {
            window.ViewImage.display(urls.length ? urls : [entry.url], entry.url);
        } else {
    ...
    function truncateColumns(depth) {
        while (columns.length > depth) {
            columns.pop().el.remove();
        }
    }

    async function pushColumn(path) {
        const depth = columns.length;
        const data = await apiList(path, 0);
        const column = buildColumn(data, depth);
        columns.push(column);
        els.columns.appendChild(column.el);
        return column;
    }

    /**
     * 打开一个目录路径:祖先链上每一级都成为一列。失败时停在最后一个
     * 还能解析的祖先上,而不是把整个视图清空 —— 路径失效多半是别处
     * (网页 / 客户端)刚改过相册,退到父目录正好是用户想看的下一步。
     */
    async function navigateTo(path, { quiet = false } = {}) {
        const chain = ['/'];
        let current = '';
        ...
        truncateColumns(0);
        // 首次进来这里还挂着 PHP 渲染的载入占位,失败过一次则是错误提示。
        els.columns.textContent = '';
        selection.clear();
        selectionDepth = -1;

        for (const step of chain) {
            try {
                await pushColumn(step);
            } catch (error) {
        ...
        if (!columns.length) {
            ...
            failed.className = 'fb-loading';
            failed.textContent = '无法读取目录';
            els.columns.appendChild(failed);
            return;
        }
    ...
    async function openFolder(depth, path) {
        truncateColumns(depth + 1);
        selection.clear();
        selectionDepth = -1;
        try {
            await pushColumn(path);
        } catch (error) {
            notify(error.message, 'error');
            return;
        }
    ...
    async function reloadAll() {
        const path = columns[columns.length - 1]?.path || '/';
        const keep = new Set(selection);
        const depth = selectionDepth;
        await navigateTo(path, { quiet: true });
        // 还活着的选中项恢复回来,被移走/删掉的自然消失。
        if (depth >= 0 && columns[depth]) {
            selectionDepth = depth;
            const alive = new Set(columns[depth].entries.map((entry) => entry.path));
            selection = new Set([...keep].filter((item) => alive.has(item)));
            if (!selection.size) selectionDepth = -1;
            paintSelection();
        }
    }
    ...
    function syncHash() {
        const path = columns[columns.length - 1]?.path || '/';
        const next = path === '/' ? '' : `#${encodeURI(path)}`;
    ...
    function handleRowClick(event, li) {
        const depth = Number(li.dataset.depth);
        const column = columns[depth];
        if (!column) return;

        const entry = column.entries.find((item) => item.path === li.dataset.path);
        if (!entry) return;
        ...
        if (entry.is_collection) {
            ...
            selectionDepth = -1;
            paintSelection();
            openFolder(depth, entry.path);
            return;
        }

        // 换列就重置:跨列多选在列视图里没有可表达的语义。
        ...
        const ranged = event.shiftKey && lastClickedPath;

        if (ranged) {
            const paths = column.entries.map((item) => item.path);
            ...
            if (from >= 0 && to >= 0) {
                ...
                for (let i = start; i <= end; i += 1) {
                    if (!column.entries[i].is_collection) selection.add(paths[i]);
                }
            }
        } else if (additive) {
    ...
    function handleDragStart(event, li) {
        const path = li.dataset.path;
        const depth = Number(li.dataset.depth);

        // 拖已选中的项就拖整组,拖组外的项则先把它变成唯一选中项 ——
        // 这是各家文件管理器一致的行为,少了它多选拖拽会莫名只动一个。
        if (selectionDepth === depth && selection.has(path)) {
            dragPaths = [...selection];
        } else {
            dragPaths = [path];
            selection = new Set(dragPaths);
            selectionDepth = depth;
            paintSelection();
        }
        ...
        columns.forEach((column) => {
            column.listEl.querySelectorAll('.fb-item').forEach((row) => {
                row.classList.toggle('is-dragging', dragPaths.includes(row.dataset.path));
            });
        });
    }

    function clearDragPaint() {
        document.querySelectorAll('.fb-item.is-dragging, .fb-item.is-drop-target')
            .forEach((row) => row.classList.remove('is-dragging', 'is-drop-target'));
    }

    function dropTargetFor(li) {
        if (!li) return null;
        const depth = Number(li.dataset.depth);
        const entry = columns[depth]?.entries.find((item) => item.path === li.dataset.path);
        if (!entry || !isDropTarget(entry)) return null;
        // 拖进自己所在的目录没有意义,也别给出可投放的假象。
        if (dragPaths.some((path) => parentPath(path) === entry.path)) return null;
        if (dragPaths.includes(entry.path)) return null;
        return entry;
    }

    async function performDrop(target, isCopy) {
        const paths = [...dragPaths];
        if (!paths.length) return;
        await runExclusive(async () => {
            const data = await apiPost(isCopy ? 'copy' : 'move', { paths, to: target.path });
            reportOutcome(data.done || 0, data.failed || [], isCopy ? '复制' : '移动');
            await reloadAll();
        });
    }
    ...
    async function deleteSelection() {
        ...
        const folders = picked.filter((entry) => entry.is_collection);
        const files = picked.filter((entry) => !entry.is_collection);

        let message;
        if (folders.length && files.length) {
            message = `将删除 ${folders.length} 个相册和 ${files.length} 张图片。相册删除后其中的图片回到「${DIR_UNFILED}」；若某张图还在其他相册（经复制链接），只取消这一处引用，否则会真正删图。`;
        } else if (folders.length) {
            message = `将删除 ${folders.length} 个相册。相册里的图片不会被删除，会回到「${DIR_UNFILED}」。`;
        } else if (files.length === 1) {
            message = `将删除「${files[0].name}」。若它还在其他相册（经复制链接），只取消这一处引用；否则原图、缩略图和远程副本都会一并清理。`;
        } else {
            message = `将删除 ${files.length} 张图片。若某张图还在其他相册（经复制链接），只取消这一处引用；否则会真正删图。`;
        }

        const ok = await window.ImgEt.DialogManager.confirm('确认删除', message, { danger: true });
        if (!ok) return;

        await runExclusive(async () => {
            const data = await apiPost('delete', { paths: picked.map((entry) => entry.path) });
            reportOutcome(data.deleted || 0, data.failed || [], '删除');
            selection.clear();
            selectionDepth = -1;
            await reloadAll();
        });
    ...
    /**
     * 就地改名 —— 把行里的名字换成 input。比弹窗贴近文件管理器的手感,
     * 也顺便避免了「弹窗里看不到同目录其它名字」导致的重名。
     */
    function startRename(entry) {
        const li = els.columns.querySelector(`.fb-item[data-path="${CSS.escape(entry.path)}"]`);
        const nameEl = li?.querySelector('.fb-item-name');
        if (!nameEl || nameEl.querySelector('input')) return;

        const input = document.createElement('input');
        input.type = 'text';
        input.className = 'fb-item-input';
        input.value = entry.name;
        nameEl.textContent = '';
        nameEl.appendChild(input);
        input.focus();
        // 选中主干、留下扩展名,连续改名时不用每次重打后缀。
        const dot = entry.name.lastIndexOf('.');
        input.setSelectionRange(0, dot > 0 && !entry.is_collection ? dot : entry.name.length);
        ...
        const finish = async (commit) => {
            ...
            await runExclusive(async () => {
                await apiPost('rename', { path: entry.path, name: next });
                notify('已重命名', 'success');
                await reloadAll();
            });
        };
    ...
    function createFolder() {
        ...
        // 顶层临时行:输入即建,取消即消失。新相册默认「仅自己」可见,
        // 跟 WebDAV 的 MKCOL 一致。
        const li = document.createElement('li');
        li.className = 'fb-item is-dir';
        const icon = makeIconEl('fa-folder-plus');
        const wrap = document.createElement('span');
        wrap.className = 'fb-item-name';
        const input = document.createElement('input');
        input.type = 'text';
        input.className = 'fb-item-input';
        input.placeholder = '新相册名称';
        wrap.appendChild(input);
        li.append(icon, wrap);
        root.listEl.prepend(li);
        input.focus();
        ...
        const finish = async (commit) => {
            ...
            await runExclusive(async () => {
                await apiPost('folder', { name });
                notify(`已创建相册「${name}」`, 'success');
                await navigateTo(`/${name}`, { quiet: true });
            });
        };
    ...
    function moveSelection(step) {
        const column = columns[selectionDepth >= 0 ? selectionDepth : columns.length - 1];
        if (!column || !column.entries.length) return;

        const depth = columns.indexOf(column);
        const current = [...selection][selection.size - 1];
        const index = column.entries.findIndex((entry) => entry.path === current);
        const next = index < 0
            ? (step > 0 ? 0 : column.entries.length - 1)
            : Math.max(0, Math.min(column.entries.length - 1, index + step));
        const entry = column.entries[next];
        if (!entry) return;
        ...
        els.columns
            .querySelector(`.fb-item[data-path="${CSS.escape(entry.path)}"]`)
            ?.scrollIntoView({ block: 'nearest' });
    }

    function handleKeydown(event) {
        if (event.target.matches('input, textarea, select')) return;
        if (document.querySelector('.confirm-dialog, .custom-dialog')) return;
        ...
        switch (event.key) {
            ...
            case 'ArrowRight': {
                ...
                if (picked.length === 1 && picked[0].is_collection) {
                    event.preventDefault();
                    openFolder(selectionDepth, picked[0].path);
                }
                break;
            }
            case 'ArrowLeft': {
                ...
                if (deepest && deepest.path !== '/') {
                    event.preventDefault();
                    navigateTo(parentPath(deepest.path));
                }
                break;
            }
            case 'Enter': {
                ...
                if (picked.length === 1) {
                    event.preventDefault();
                    if (picked[0].is_collection) {
                        openFolder(selectionDepth, picked[0].path);
                    } else if (!picked[0].missing && picked[0].url) {
                        openLightbox(picked[0]);
                    }
                }
                break;
            }
            case 'F2': {
                const picked = selectedEntries();
                if (picked.length === 1 && picked[0].writable) {
                    event.preventDefault();
                    startRename(picked[0]);
                }
                break;
            }
            case 'Escape':
    ...
    // 行是动态生成的,所以全部走列容器上的委托。
    els.columns.addEventListener('click', (event) => {
        const li = event.target.closest('.fb-item');
        if (!li || !li.dataset.path) return;
        handleRowClick(event, li);
    });
    ...
    els.columns.addEventListener('dragend', () => {
        dragPaths = [];
        clearDragPaint();
    });
    ...
    els.columns.addEventListener('dragover', (event) => {
        ...
        const target = dropTargetFor(event.target.closest('.fb-item'));
        if (!target) return;
        event.preventDefault();
        event.dataTransfer.dropEffect = event.altKey ? 'copy' : 'move';
        document.querySelectorAll('.fb-item.is-drop-target')
            .forEach((row) => row.classList.remove('is-drop-target'));
        event.target.closest('.fb-item').classList.add('is-drop-target');
    });

    els.columns.addEventListener('dragleave', (event) => {
        event.target.closest('.fb-item')?.classList.remove('is-drop-target');
    });

    els.columns.addEventListener('drop', (event) => {
        const li = event.target.closest('.fb-item');
        const target = dropTargetFor(li);
        if (!target) return;
        event.preventDefault();
        const isCopy = event.altKey;
        clearDragPaint();
        performDrop(target, isCopy);
        dragPaths = [];
    });
    ...
    els.refreshBtn?.addEventListener('click', () => runExclusive(() => reloadAll()));

    document.addEventListener('keydown', handleKeydown);
    window.addEventListener('hashchange', () => {
        const wanted = pathFromHash();
        if (wanted !== (columns[columns.length - 1]?.path || '/')) {
            navigateTo(wanted, { quiet: true });
        }
    });

    navigateTo(pathFromHash(), { quiet: true });
})();
