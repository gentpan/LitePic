...
[![Version](https://img.shields.io/badge/version-3.4.4-0052D9?style=flat-square)](https://github.com/gentpan/LitePic/releases)
...
| Web 服务器 | FrankenPHP（内置 Caddy） | FrankenPHP 经典模式 |
...
```bash
# 推荐：FrankenPHP（改 Caddyfile 里的域名 / root，或设 SERVER_NAME）
frankenphp run --config ./Caddyfile

# 可选：Nginx + PHP-FPM，参考 nginx-litepic.conf
```

打开浏览器访问站点域名，第一次进设置页面注册管理员账号即可开始上传。
...
├── Caddyfile             # FrankenPHP / Caddy 参考配置（推荐）
├── nginx-litepic.conf    # Nginx + PHP-FPM 参考配置（可选）
