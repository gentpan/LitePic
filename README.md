...
[![Version](https://img.shields.io/badge/version-3.4.1-0052D9?style=flat-square)](https://github.com/gentpan/LitePic/releases)
...
- Worker 三种触发方式（响应后 drain / Cron 兜底 / 手动触发），同一份 SQLite，flock 互斥
...
| Web 服务器 | Apache / Nginx / OpenResty / Caddy / LiteSpeed / 内置 | Nginx + PHP-FPM |
...
# 生产：参考 nginx-litepic.conf，改 root + server_name 后 reload
```

打开浏览器访问域名或 `http://127.0.0.1:5555`，第一次进设置页面注册管理员账号即可开始上传。
